...
class BayesianNetwork:
    ...
    def get_cpd_bin(self, n, val, gatetype):
        if n > 1:
            if gatetype == 'and' or not gatetype:
    ...
    def get_cpd_var(self, name, parent):
        ...
        try:
            ...
            pclass = 'Other'
        if pclass not in self.dat['matrices'][0].keys():
            pclass = 'Other'
        if fclass not in self.dat['matrices'][0][pclass].keys():
            fclass = 'Other'
        cpdy = self.dat['matrices'][0][pclass][fclass].values()
        if pclass not in self.dat['matrices'][1].keys():
            pclass = 'Other'
        if fclass not in self.dat['matrices'][1][pclass].keys():
            fclass = 'Other'
