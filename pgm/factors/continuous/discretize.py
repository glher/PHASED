...
class BaseDiscretizer(with_metaclass(ABCMeta)):
    """
    Base class for the discretizer classes in pgm. The discretizer
    ...
    >>> from pgm.factors.continuous import ContinuousNode
    >>> normal = ContinuousNode(norm(0, 1).pdf)
    >>> from pgm.discretize import BaseDiscretizer
    >>> class ChildDiscretizer(BaseDiscretizer):
    ...     def get_discrete_values(self):
    ...         pass
    >>> discretizer = ChildDiscretizer(normal, -3, 3, 10)
    >>> discretizer.factor
    <pgm.factors.continuous.ContinuousNode.ContinuousNode object at 0x04C98190>
    ...
    def get_labels(self):
        ...
        Examples
        --------
        >>> from pgm.factors import ContinuousNode
        >>> from pgm.discretize import BaseDiscretizer
        >>> class ChildDiscretizer(BaseDiscretizer):
        ...     def get_discrete_values(self):
        ...         pass
...
class RoundingDiscretizer(BaseDiscretizer):
    ...
    >>> import numpy as np
    >>> from pgm.factors.continuous import ContinuousNode
    >>> from pgm.factors.continuous import RoundingDiscretizer
...
class UnbiasedDiscretizer(BaseDiscretizer):
    ...
    >>> import numpy as np
    >>> from pgm.factors import ContinuousNode
    >>> from pgm.factors.continuous import UnbiasedDiscretizer
