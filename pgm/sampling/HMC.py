# -*- coding: UTF-8 -*-
"""
    A collection of methods for sampling from continuous models in pgm
"""
...
from pgm.utils import _check_1d_array_object, _check_length_equal
from pgm.sampling import LeapFrog, BaseSimulateHamiltonianDynamics, BaseGradLogPDF
...
class HamiltonianMC(object):
    ...
    Parameters:
    -----------
    model: An instance pgm.models
        Model from which sampling has to be done

    grad_log_pdf: A subclass of pgm.inference.continuous.BaseGradLogPDF, defaults to None
    ...
    simulate_dynamics: A subclass of pgm.inference.continuous.BaseSimulateHamiltonianDynamics
    ...
    Example:
    --------
    >>> from pgm.sampling import HamiltonianMC as HMC, LeapFrog, GradLogPDFGaussian
    >>> from pgm.factors.continuous import JointGaussianDistribution as JGD
    >>> import numpy as np
    >>> mean = np.array([-3, 4])
    ...
    def sample(self, initial_pos, num_samples, trajectory_length, stepsize=None):
        ...
        >>> from pgm.sampling import HamiltonianMC as HMC, GradLogPDFGaussian, ModifiedEuler
        >>> from pgm.factors.continuous import JointGaussianDistribution as JGD
        >>> import numpy as np
        >>> mean = np.array([1, -1])
    ...
    def generate_sample(self, initial_pos, num_samples, trajectory_length, stepsize=None):
        ...
        Examples
        --------
        >>> from pgm.inference.continuous import HamiltonianMC as HMC, GradLogPDFGaussian as GLPG
        >>> from pgm.factors import JointGaussianDistribution as JGD
        >>> import numpy as np
        >>> mean = np.array([4, -1])
...
class HamiltonianMCDA(HamiltonianMC):
    ...
    grad_log_pdf: A subclass of pgm.inference.continuous.GradientLogPDF
    ...
    Example:
    --------
    >>> from pgm.sampling import HamiltonianMCDA as HMCda, LeapFrog
    >>> from pgm.factors.continuous import JointGaussianDistribution as JGD
    >>> import numpy as np
    >>> mean = np.array([1, 2, 3])
    ...
    def sample(self, initial_pos, num_adapt, num_samples, trajectory_length, stepsize=None):
        ...
        Examples
        ---------
        >>> from pgm.sampling import HamiltonianMCDA as HMCda, GradLogPDFGaussian as GLPG, LeapFrog
        >>> from pgm.factors.continuous import JointGaussianDistribution as JGD
        >>> import numpy as np
        >>> mean = np.array([1, 1])
