...
class BaseGradLogPDF(object):
    ...
    model : An instance of pgm.models

    Examples
    --------
    >>> from pgm.factors import JointGaussianDistribution
    >>> from pgm.inference.continuous import BaseGradLogPDF
    >>> import numpy as np
    ...
    def get_gradient_log_pdf(self):
        ...
        >>> from pgm.sampling.base import GradLogPDFGaussian
        >>> from pgm.factors.continuous import JointGaussianDistribution
        >>> import numpy as np
        >>> mean = np.array([1, 1])
...
class GradLogPDFGaussian(BaseGradLogPDF):
    ...
    Inherits pgm.inference.base_continuous.BaseGradLogPDF
    ...
    model : An instance of pgm.models.JointGaussianDistribution

    Example
    -------
    >>> from pgm.sampling import GradLogPDFGaussian
    >>> from pgm.factors.continuous import JointGaussianDistribution
    >>> import numpy as np
    >>> mean = np.array([3, 4])
...
class BaseSimulateHamiltonianDynamics(object):
    ...
    grad_log_pdf : A subclass of pgm.inference.continuous.BaseGradLogPDF
    ...
    Examples
    --------
    >>> from pgm.sampling import BaseSimulateHamiltonianDynamics
    >>> from pgm.factors.continuous import JointGaussianDistribution
    >>> from pgm.sampling import GradLogPDFGaussian
    >>> import numpy as np
    ...
    def __init__(self, model, position, momentum, stepsize, grad_log_pdf, grad_log_position=None):
        ...
        if not issubclass(grad_log_pdf, BaseGradLogPDF):
            raise TypeError("grad_log_pdf must be an instance" +
                            " of pgm.inference.continuous.base.BaseGradLogPDF")
    ...
    def get_proposed_values(self):
        ...
        >>> from pgm.inference.continuous import ModifiedEuler, GradLogPDFGaussian as GLPG
        >>> from pgm.factors import JointGaussianDistribution
        >>> import numpy as np
        >>> pos = np.array([3, 4])
...
class LeapFrog(BaseSimulateHamiltonianDynamics):
    ...
    Inherits pgm.inference.base_continuous.BaseSimulateHamiltonianDynamics
    ...
    --------
    >>> from pgm.factors.continuous import JointGaussianDistribution
    >>> from pgm.sampling import LeapFrog, GradLogPDFGaussian as GLPG
    >>> import numpy as np
    >>> pos = np.array([2, 1])
...
class ModifiedEuler(BaseSimulateHamiltonianDynamics):
    ...
    --------
    >>> from pgm.factors.continuous import JointGaussianDistribution
    >>> from pgm.sampling import GradLogPDFGaussian, ModifiedEuler
    >>> import numpy as np
    >>> pos = np.array([2, 1])
    >>> momentum = np.array([1, 1])
    >>> mean = np.array([0, 0])
