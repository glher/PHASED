...
from pgm.extern.six.moves import map
...
def generate_probmodelxml(model, encoding='utf-8', prettyprint=True):
    ...
    --------
    >>> G = nx.path_graph(5)
    >>> s = pgm.readwrite.generate_ProbModelXML(G) # doctest: +SKIP
    >>> for line in pgm.readwrite.generate_ProbModelXML(G):  #doctest: +SKIP
    ...     print(line)
    """
...
def write_probmodelxml(model, path, encoding='utf-8', prettyprint=True):
    ...
    --------
    >>> G = nx.path_graph(4)
    >>> pgm.readwrite.write_probmodelxml(G, "test.probmodelxml")
    """
...
def read_probmodelxml(path):
    ...
    Examples
    --------
    >>> G = pgm.readwrite.read_probmodelxml('test.probModelXML')
    """
...
def parse_probmodelxml(string):
    ...
    >>> G = nx.path_graph(4)
    >>> linefeed = chr(10)
    >>> s = linefeed.join(pgm.readwrite.generate_probmodelxml(G))
    >>> H = pgm.readwrite.parse_probmodelxml(s)
    """
...
def get_probmodel_data(model):
    ...
    Examples
    --------
    >>> model_data = pgm.readwrite.get_model_data(model)
